...
import { buildCanvasTools, buildCanvasSystemPrompt, type CanvasContext } from './canvas-tools.js'
import { makeCanvasExecutor } from './canvas-executor.js'
...
// Allowlist of models the client may select (`ALLOWED_MODELS`) + the balanced
// default (`DEFAULT_MODEL`) both live in `./models.ts` — the single source of
// truth shared with the picker UI (AiAssistant / ChatPanel) so the two can
// never drift (drift used to surface as silent 400s "Unknown modelId").
...
// Cap on the number of shapes the client may send as canvas context. The full
// shape list is folded into the system prompt, so an unbounded array would
// balloon the prompt (and cost). Far above any realistic hand-built canvas.
const MAX_CANVAS_SHAPES = 1_000

/**
 * Validate + normalize an optional `canvasContext` from the request body into
 * a `CanvasContext`, or `null` when absent. Returns a string on a shape error
 * so the route can 400 with a clear message. Caps the shapes array length.
 */
function parseCanvasContext(
  raw: unknown,
): { ctx: CanvasContext } | { error: string } | { ctx: null } {
  if (raw === undefined || raw === null) return { ctx: null }
  if (typeof raw !== 'object') return { error: 'canvasContext must be an object' }

  const obj = raw as Record<string, unknown>
  if (typeof obj.docId !== 'string' || obj.docId === '') {
    return { error: 'canvasContext.docId is required' }
  }
  if (!Array.isArray(obj.shapes)) {
    return { error: 'canvasContext.shapes must be an array' }
  }
  if (obj.shapes.length > MAX_CANVAS_SHAPES) {
    return { error: `canvasContext.shapes exceeds ${MAX_CANVAS_SHAPES} items` }
  }
  if (!Array.isArray(obj.selectedShapeIds)) {
    return { error: 'canvasContext.selectedShapeIds must be an array' }
  }

  const shapes: CanvasContext['shapes'] = []
  for (const s of obj.shapes) {
    if (s === null || typeof s !== 'object') {
      return { error: 'canvasContext.shapes entries must be objects' }
    }
    const sr = s as Record<string, unknown>
    // Style/text fields are optional and strictly validated: only strings pass
    // through, so a malformed payload can't smuggle objects into the prompt.
    ...
    if (typeof sr.fill === 'string') entry.fill = sr.fill
    if (typeof sr.stroke === 'string') entry.stroke = sr.stroke
    if (typeof sr.text === 'string') entry.text = sr.text
    shapes.push(entry)
  }
  const selectedShapeIds = obj.selectedShapeIds
    .filter((id): id is string => typeof id === 'string')

  return { ctx: { docId: obj.docId, shapes, selectedShapeIds } }
}
...
export function registerAiChatRoutes(
  ...
  app.post('/api/ai/chat', async (c) => {
    ...
    const { chatId, userMessageId, content, modelId, canvasContext } = await c.req.json<{
      chatId?: string
    ...
    // Optional canvas context — when present, the assistant gets the canvas
    // tools + a system-prompt block describing the current shapes/selection.
    const canvasParsed = parseCanvasContext(canvasContext)
    if ('error' in canvasParsed) {
      return c.json({ error: canvasParsed.error }, 400)
    }
    const canvasCtx = canvasParsed.ctx
    ...
    const summarySystem = summary ? `${baseSystem}\n\n${summary.content}` : baseSystem
    // Fold the canvas instructions (tools, coordinate space, current shapes +
    // selection) into the system prompt when the request carries canvas context.
    const systemText = canvasCtx
      ? `${summarySystem}\n\n${buildCanvasSystemPrompt(canvasCtx)}`
      : summarySystem
    ...
    // When canvas context is present, register the canvas tools alongside the
    // record tools. Their executor is VALIDATE-ONLY (`makeCanvasExecutor`) — it
    // normalizes the requested op and echoes it back so the model can chain
    // multi-step drawing, but it NEVER mutates the canvas. The client watches
    // the same tool-call stream and is the single writer to `useCanvas`.
    if (canvasCtx) {
      Object.assign(tools, buildCanvasTools(makeCanvasExecutor(canvasCtx)))
    }
    ...
    const result = streamText({
      ...
      messages,
      tools,
      // Cap the multi-step tool loop. The canvas batch tools (canvas_drawDiagram
      // / canvas_createShapes) build a whole diagram in a single call, so a turn
      // rarely needs many steps — but allow headroom for the model to draw, read
      // back, and adjust. Each step is a full LLM round-trip, so don't set this
      // arbitrarily high. v4's `maxSteps` shape; v5 expresses it as `stopWhen`.

...
export function buildSystemPrompt(appName: string, schemas: CollectionSchema[]): string {
  ...
  return [
    ...
    'Match the response to the request: keep a simple ask simple. Give the',
    'straightforward answer first; do not over-engineer, take extra steps, or',
    'make data changes the user did not ask for. (This is about scope, not',
    'visual richness — when the user asks for a drawing, follow the drawing',
    'guidance and include the detail that makes it look finished.)',
    '',
